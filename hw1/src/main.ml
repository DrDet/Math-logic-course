...
let annotate s n = 
	...
	let buf = Buffer.create 100 in
		...
		if hyp > 0 then 
			begin
			add_string buf " (Предп. "; add_string buf (string_of_int hyp); add_string buf ")"
			end
		else if ax > 0 then
			begin
			add_string buf " (Сх. акс. "; add_string buf (string_of_int ax); add_string buf ")"
			end
		else if i > 0 && j > 0 then
			begin
			add_string buf " (M.P. "; add_string buf (string_of_int i); add_string buf ", "; add_string buf (string_of_int j); add_string buf ")"
			end
		else
			add_string buf " (Не доказано)"
		;
		upd_mp tree n;
		contents buf
;;
